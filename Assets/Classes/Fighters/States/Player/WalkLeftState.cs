...
public class WalkLeftState : IState
{
    ...
    public WalkLeftState(PlayerFighterController stateController, StateMachine stateMachine)
    {
    ...
    {
        if (input == InputValue.RIGHT_PRESS)
        {
            stateMachine.ChangeState(fighterController.walkRightState);
        }
        else if (input == InputValue.LEFT_RELEASE)
        {
